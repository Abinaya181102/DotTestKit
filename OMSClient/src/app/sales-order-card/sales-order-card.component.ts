...
export class SalesOrderCardComponent implements OnInit {
  ...
  setAddressesForCustomer() {
    this.dataService
      .getAddressesForCustomer(this.order.customerId)
      .subscribe((response) => {
  ...
  insertNewLine() {
    ...
    this.dataService
      .newSalesOrderLine(salesOrderLineCreate)
      .subscribe((response) => {
        if (response.ok) {
  ...
  validate() {
    let valid = true;
    if (this.order.orderDate === undefined) {
      valid = false;
    }
    if (this.order.shipmentDate === undefined) {
      valid = false;
    }
    if (this.order.customerId === undefined) {
      valid = false;
    }
    if (this.order.addressId === undefined) {
      valid = false;
    }
    if (this.order.shipmentDate < this.order.orderDate) {
      valid = false;
      this.showSnackBar(
        "Shipment Date is before Order Date. Cannot update the order."
      );
    }
    return valid;
  }
  ...
  updateOrder() {
    this.dataService
      .updateSalesOrder(this.order.id, this.order as SalesOrderHeaderUpdate)
      .subscribe((response) => {
        if (response.ok) {
          this.transferOrderFields(this.order, this.xOrder);
  ...
  deleteLine(lineId: number) {
    this.dataService.deleteSalesOrderLine(lineId).subscribe((status) => {
      ...
      const oldData = this.linesData.data;
      oldData.splice(
        this.linesData.data.findIndex((line) => line.id === lineId),
        1
      );
      this.linesData.data = oldData;
    });
  ...
  updateProfit() {
    this.dataService
      .updateSalesOrderProfit(this.order.id)
      .subscribe((response) => {
        if (response.ok) {
  ...
  private transferOrderFields(
    from: SalesOrderHeaderReadFull,
    to: SalesOrderHeaderReadFull
  ) {
    to.id = from.id;
